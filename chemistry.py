def main():
    ...
    # list that stores element symbols and the quantityet
    ...
    formula_name_dict = molecules_name()
    formula = input("Enter the molecular formula of the sample: ")
    mass_in_grams = float(input("Enter the mass in grams of the sample: "))
    formula_name = get_formula_name(formula, formula_name_dict)
    get_formula = parse_formula(formula, table)
    molar_mass_calculator = compute_molar_mass(get_formula, table)
    moles = mass_in_grams/ molar_mass_calculator
    print("Results:")
    print(f"\nThe molar mass of glucose: {molar_mass_calculator:.5f} grams/mole")
...
def get_formula_name(formula, known_molecules_dict):
    ...
    if formula in known_molecules_dict:
        name = f"{formula} is a formula of: {known_molecules_dict[formula]}"
        return name 

    else:
        name = "This an unknown compound"
        return name   
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for x in symbol_quantity_list:
        symbol = x[0]
        quantity = x[1]
        name = periodic_table_dict[symbol]  [0]
        mass = periodic_table_dict[symbol] [1]
        atomic_mass_by_quantity = mass * quantity
        total_molar_mass += atomic_mass_by_quantity      
        
        print(f"{mass} is the atomic mass in {name}")          
